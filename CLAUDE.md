...
- #25 일괄 업로드 화면 구현 (`/songs/bulk`, 드래그앤드롭 + OCR 자동분석 + 카드별 편집 + 전체 저장)

### ✅ 추가 완료
- #27 가사 관리 (songs.lyrics 컬럼 추가, PATCH /lyrics 엔드포인트, 곡 상세 가사 섹션 — OCR 적용 + 직접 편집)
- #28 통합 검색 (제목·아티스트·가사 LIKE 검색 + 코드 대소문자 무관 LIKE 필터)
